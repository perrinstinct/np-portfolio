...
export function TrustBar() {
  const { trustedBy, techMarquee, ui } = useContent()
    ...
    <section className="border-y border-line bg-white py-12">
      <div className="container-content">
        <Reveal className="flex flex-col items-center gap-8">
          <p className="font-mono text-xs uppercase tracking-[0.2em] text-ink-400">{ui.trustedBy}</p>
          <div className="flex flex-wrap items-center justify-center gap-x-12 gap-y-6">
            {trustedBy.map((c) => (
              <div key={c.name} className="flex items-center gap-2.5" title={c.name}>
                <span
                  className="logo-mono h-7 w-8"
                  style={{ WebkitMaskImage: `url(${c.logo})`, maskImage: `url(${c.logo})` }}
                  role="img"
                  aria-label={`${c.name} logo`}
                />
                <span className="font-display text-lg font-700 tracking-tight text-ink-700">{c.name}</span>
                {c.note && (
